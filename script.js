...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Create and handle the settings panel for profile editing
  function createSettingsPanel() {
    // Overlay that darkens the background and holds the settings panel
    const overlay = document.createElement('div');
    overlay.id = 'settings-overlay';
    overlay.className = 'modal-overlay';
    overlay.style.display = 'none';
    // Panel containing the form and actions
    ...
    panel.innerHTML = `
      ...
      <form id="profile-form" class="form-row">
        <input type="text" id="profile-name" placeholder="Nom complet" required />
        <input type="email" id="profile-email" placeholder="Adresse e‑mail" required />
        <button type="submit">Enregistrer</button>
      </form>
      <button id="logout-button" class="secondary" style="margin-top:0.5rem;">Déconnexion</button>
    `;
    ...
    const profileForm = panel.querySelector('#profile-form');
    profileForm.addEventListener('submit', (e) => {
      ...
      Promise.all(promises)
        .then(() => {
          if (userDocRef) {
            return userDocRef.update({ name: newName, email: newEmail });
          }
        })
        .catch((err) => {
  ...
  // Instantiate the settings panel immediately
  createSettingsPanel();
    ...
    if (tapButton) {
      tapButton.addEventListener('click', () => {
        ...
        if (tapTimes.length >= 2) {
          ...
          const clamped = Math.min(Math.max(bpmFromTap, minBpm), maxBpm);
